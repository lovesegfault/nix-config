{ config, pkgs, ... }: {
  ...
  services.nginx.virtualHosts = {
    "deluge.meurer.org" = {
      locations = {
        "@error401".extraConfig = ''
          return 302 https://vouch.meurer.org/login?url=$scheme://$host$request_uri&vouch-failcount=$auth_resp_failcount&X-Vouch-Token=$auth_resp_jwt&error=$auth_resp_err;
        '';
    ...
    "vouch.meurer.org" = {
      ...
      forceSSL = true;
      kTLS = true;
      locations."/".proxyPass = "http://127.0.0.1:30746";
    };
  };
